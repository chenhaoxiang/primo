...
//		_table.draw(false);
    ...
    function getTableData(param,callback){
        ...
        $.ajax({
            type: "POST",
            url: "/primo/urlruleprocessorconfig/page",
            cache: false,	//禁用缓存
            data: param,	//传入已封装的参数
            dataType: "json",
            //RequestBody 传参
    ...
    function addItem(parameter){
        ajaxPostRequestBody("/primo/urlruleprocessorconfig/save",JSON.stringify(parameter),resetFormData,true,$("#form-add")[0]);
    ...
    function editItem(parameter){
        //表单重置,应该是请求成功再重置
        ajaxPostRequestBody("/primo/urlruleprocessorconfig/update",JSON.stringify(parameter),resetFormData,true,$("#form-edit")[0]);
    }

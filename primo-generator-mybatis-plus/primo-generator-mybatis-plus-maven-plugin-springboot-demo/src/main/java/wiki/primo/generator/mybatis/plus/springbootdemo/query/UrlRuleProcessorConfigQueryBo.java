 ...
 */
public class UrlRuleProcessorConfigQueryBo{
	/**
	 * 排序，批量查询
	 */
	 ...
	 * @return
	 */
	public QueryWrapper<UrlRuleProcessorConfig> buildQuery() {
		QueryWrapper<UrlRuleProcessorConfig> query = new QueryWrapper<>();

		if(id!=null){
			query.eq(UrlRuleProcessorConfig.ID,id);
		}
		if(platformNameEn!=null){
			query.eq(UrlRuleProcessorConfig.PLATFORM_NAME_EN,platformNameEn);
		}
		if(referer!=null){
			query.eq(UrlRuleProcessorConfig.REFERER,referer);
		}
		if(urlPrefix!=null){
			query.eq(UrlRuleProcessorConfig.URL_PREFIX,urlPrefix);
		}
		if(urlRuleJson!=null){
			query.eq(UrlRuleProcessorConfig.URL_RULE_JSON,urlRuleJson);
		}
		if(titleRuleJson!=null){
			query.eq(UrlRuleProcessorConfig.TITLE_RULE_JSON,titleRuleJson);
		}
		if(pulishTimeRuleJson!=null){
			query.eq(UrlRuleProcessorConfig.PULISH_TIME_RULE_JSON,pulishTimeRuleJson);
		}
		if(authorRuleJson!=null){
			query.eq(UrlRuleProcessorConfig.AUTHOR_RULE_JSON,authorRuleJson);
		}
		if(tagsRuleJson!=null){
			query.eq(UrlRuleProcessorConfig.TAGS_RULE_JSON,tagsRuleJson);
		}
		if(dateDelete!=null){
			query.eq(UrlRuleProcessorConfig.DATE_DELETE,dateDelete);
		}
		if(domain!=null){
			query.eq(UrlRuleProcessorConfig.DOMAIN,domain);
		}
		if(blogsAuthorIndexUrlType!=null){
			query.eq(UrlRuleProcessorConfig.BLOGS_AUTHOR_INDEX_URL_TYPE,blogsAuthorIndexUrlType);
		}
		if(createTime!=null){
			query.eq(UrlRuleProcessorConfig.CREATE_TIME,createTime);
		}
		if(updateTime!=null){
			query.eq(UrlRuleProcessorConfig.UPDATE_TIME,updateTime);
		}
		if(status!=null){
			query.eq(UrlRuleProcessorConfig.STATUS,status);
		}
		...
		if(queryBoExt!=null){
			...
			//批量的查询条件进行查询
                ...
                query.in(UrlRuleProcessorConfig.ID,queryBoExt.getIdList());
            }
                        if(queryBoExt.getPlatformNameEnList()!=null && queryBoExt.getPlatformNameEnList().size()>0){
                query.in(UrlRuleProcessorConfig.PLATFORM_NAME_EN,queryBoExt.getPlatformNameEnList());
            }
                        if(queryBoExt.getRefererList()!=null && queryBoExt.getRefererList().size()>0){
                query.in(UrlRuleProcessorConfig.REFERER,queryBoExt.getRefererList());
            }
                        if(queryBoExt.getUrlPrefixList()!=null && queryBoExt.getUrlPrefixList().size()>0){
                query.in(UrlRuleProcessorConfig.URL_PREFIX,queryBoExt.getUrlPrefixList());
            }
                        if(queryBoExt.getUrlRuleJsonList()!=null && queryBoExt.getUrlRuleJsonList().size()>0){
                query.in(UrlRuleProcessorConfig.URL_RULE_JSON,queryBoExt.getUrlRuleJsonList());
            }
                        if(queryBoExt.getTitleRuleJsonList()!=null && queryBoExt.getTitleRuleJsonList().size()>0){
                query.in(UrlRuleProcessorConfig.TITLE_RULE_JSON,queryBoExt.getTitleRuleJsonList());
            }
                        if(queryBoExt.getPulishTimeRuleJsonList()!=null && queryBoExt.getPulishTimeRuleJsonList().size()>0){
                query.in(UrlRuleProcessorConfig.PULISH_TIME_RULE_JSON,queryBoExt.getPulishTimeRuleJsonList());
            }
                        if(queryBoExt.getAuthorRuleJsonList()!=null && queryBoExt.getAuthorRuleJsonList().size()>0){
                query.in(UrlRuleProcessorConfig.AUTHOR_RULE_JSON,queryBoExt.getAuthorRuleJsonList());
            }
                        if(queryBoExt.getTagsRuleJsonList()!=null && queryBoExt.getTagsRuleJsonList().size()>0){
                query.in(UrlRuleProcessorConfig.TAGS_RULE_JSON,queryBoExt.getTagsRuleJsonList());
            }
                        if(queryBoExt.getDateDeleteList()!=null && queryBoExt.getDateDeleteList().size()>0){
                query.in(UrlRuleProcessorConfig.DATE_DELETE,queryBoExt.getDateDeleteList());
            }
                        if(queryBoExt.getDomainList()!=null && queryBoExt.getDomainList().size()>0){
                query.in(UrlRuleProcessorConfig.DOMAIN,queryBoExt.getDomainList());
            }
                        if(queryBoExt.getBlogsAuthorIndexUrlTypeList()!=null && queryBoExt.getBlogsAuthorIndexUrlTypeList().size()>0){
                query.in(UrlRuleProcessorConfig.BLOGS_AUTHOR_INDEX_URL_TYPE,queryBoExt.getBlogsAuthorIndexUrlTypeList());
            }
                        if(queryBoExt.getCreateTimeList()!=null && queryBoExt.getCreateTimeList().size()>0){
                query.in(UrlRuleProcessorConfig.CREATE_TIME,queryBoExt.getCreateTimeList());
            }
                        if(queryBoExt.getUpdateTimeList()!=null && queryBoExt.getUpdateTimeList().size()>0){
                query.in(UrlRuleProcessorConfig.UPDATE_TIME,queryBoExt.getUpdateTimeList());
            }
                        if(queryBoExt.getStatusList()!=null && queryBoExt.getStatusList().size()>0){
                query.in(UrlRuleProcessorConfig.STATUS,queryBoExt.getStatusList());
            }
            		}
		return query;
	}

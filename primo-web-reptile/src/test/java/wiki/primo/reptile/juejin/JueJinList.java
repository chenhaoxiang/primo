 ...
 * @author chenhx
 * @version 0.0.1
 * @className JueJinList.java
 * @date 2021-07-22 8:11 下午
 * @description
 */
public class JueJinList {
    /**
     * 获取文章列表数据
     * @param args
     */
    public static void main(String[] args) {
        String url = "https://juejin.cn/backend?sort=newest";
        ...
        try {
            ...
            //等待元素的加载。最大的时间30秒
            wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(JueJinBlog.URLS.substring(0, JueJinBlog.URLS.lastIndexOf("/")))));
        ...
        List<String> titles = jxDocument.selN(JueJinBlog.TITLES).stream().map(JXNode::asString).collect(Collectors.toList());
        List<String> blogUrls = jxDocument.selN(JueJinBlog.URLS).stream().map(JXNode::asString).collect(Collectors.toList());
        webDriver.close();
        webDriver.quit();
